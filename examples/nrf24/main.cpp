...
typedef GPIO_PORT_T<PA, NRF24_CE, NRF24_CSN, NRF24_IRQ, TX, RX, CSN, SCK, MISO, MOSI> PORT_A;
typedef GPIO_PORT_T<PB, BUTTON, LED1, LED2> PORT_B;
typedef EXTI_T<NRF24_IRQ> EXT_INTERRUPT;

typedef USART_T<USART_1, SYSCLK, 115200> UART;
typedef SPI_T<SPI_1, SYSCLK, true, 0, 500000> SPI;
typedef NRF24_T<SPI, NRF24_CSN, NRF24_CE, NRF24_IRQ, SYSTICK> NRF24;
...
int main(void)
{
	...
	hex_dump<UART>(regs, sizeof(regs));
#if (defined SENDING)
	int n = 0;
	while (1) {
		NRF24::start_tx();
